...
// ✅ ИСПРАВЛЕНЫ ВСЕ URL ПОД НОВЫЙ БЭКЕНД
async function apiCall(endpoint, method = 'GET', payload = null) {
    ...
    if (!r.ok) {
        if (r.status === 401) throw new Error("Ошибка авторизации.");
        if (r.status === 429) throw new Error("Слишком много запросов.");
...
function updateUI(d) {
    ...
    const sB = document.getElementById('btn-toggle-sleep'), sI = document.getElementById('sleep-icon'), sT = document.getElementById('sleep-btn-text'), sS = document.getElementById('sleep-status');
    if(d.sleep_start) {
        sB.classList.add('sleep-active'); sI.textContent = '☀️'; sT.textContent = 'Проснуться';
        ...
        sB.classList.remove('sleep-active'); sI.textContent = '🌙'; sT.textContent = 'Уснуть'; sS.textContent = 'OFF';
    ...
    if (d.custom_habits) {
        try {
            ...
            currentHabits = Array.isArray(parsed) ? parsed : [];
...
function renderHistory(historyData) {
    ...
    historyData.forEach(h => {
        ...
        leftDiv.appendChild(title); leftDiv.appendChild(timeDiv);
...
function renderHabits() {
    const s = document.getElementById('list-sport'), f = document.getElementById('list-family'), eList = document.getElementById('editor-list');
    s.textContent = ''; f.textContent = ''; eList.textContent = '';
    ...
    allTasks.forEach(h => {
        ...
        btn.appendChild(circle); btn.appendChild(nameDiv); btn.appendChild(xpDiv);
        btn.addEventListener('click', () => completeTask(h, btn));
            
        if (h.cat === 'sport') s.appendChild(btn); else f.appendChild(btn);
        ...
        if (h.id.startsWith('task-custom')) {
            ...
            btnGroup.appendChild(eBtn); btnGroup.appendChild(dBtn);
            card.appendChild(cName); card.appendChild(btnGroup);
            eList.appendChild(card);
        }
    });

    document.querySelectorAll('.accordion-content').forEach(el => {
        if (el.style.maxHeight && el.style.maxHeight !== '0px') el.style.maxHeight = el.scrollHeight + 'px';
    });
}
...
function renderShop() {
    ...
    serverRewards.forEach(r => {
        ...
        topFlex.appendChild(iconDiv); topFlex.appendChild(nameGroup);
        ...
        counter.appendChild(btnM); counter.appendChild(spanQ); counter.appendChild(btnP);
        ...
        botFlex.appendChild(counter); botFlex.appendChild(buyBtn);
        card.appendChild(topFlex); card.appendChild(botFlex);
...
async function loadHero(forceUpdate = false) {
    ...
    try {
        ...
        const st = document.getElementById('splash-text'), sr = document.getElementById('btn-splash-reload');
        if(st && sr) { st.textContent = e.message; st.classList.replace('text-blue-500', 'text-red-500'); st.classList.remove('animate-pulse'); sr.classList.remove('hidden'); }
    }
}
...
async function completeTask(habit, btn) {
    ...
    try {
        const payload = { task_id: habit.id };
        if(habit.id.startsWith('task-custom')) { payload.custom_name = habit.name; payload.custom_xp = habit.xp; }
        const data = await apiCall(`/tasks/complete`, 'POST', payload);
        updateUI(data);
    } catch (e) { tg.showAlert(e.message); } finally { btn.classList.remove('btn-loading'); isLocked = false; }
}

async function buyReward(rewardId) {
    if(isLocked) return; isLocked = true;
    const btn = document.getElementById(`buy-btn-${rewardId}`); btn.classList.add('btn-loading');
    if (tg.HapticFeedback) tg.HapticFeedback.impactOccurred('medium');
    try {
        const data = await apiCall(`/rewards/buy`, 'POST', { reward_id: rewardId, qty: shopState[rewardId] });
        if(data.error) tg.showAlert(data.error); else { updateUI(data); shopState[rewardId] = 1; renderShop(); }
    } catch (e) { tg.showAlert(e.message); } finally { btn.classList.remove('btn-loading'); isLocked = false; }
}

async function drinkWater() {
    if(isLocked) return; isLocked = true;
    const btn = document.getElementById('btn-drink-water'); btn.classList.add('btn-loading');
    if (tg.HapticFeedback) tg.HapticFeedback.impactOccurred('light');
    try { const data = await apiCall(`/health/drink`, 'POST'); updateUI(data); } catch (e) { tg.showAlert(e.message); } finally { btn.classList.remove('btn-loading'); isLocked = false; }
}

async function toggleSleep() {
    if(isLocked) return; isLocked = true;
    const btn = document.getElementById('btn-toggle-sleep'); btn.classList.add('btn-loading');
    try {
...
async function applyWaterGoal() {
    if(isLocked) return; isLocked = true;
    const btn = document.getElementById('btn-apply-calc'), w = document.getElementById('calc-weight').value, a = document.getElementById('calc-activity').value;
    if(w > 0) {
        btn.classList.add('btn-loading');
        ...
        try {
            ...
            updateUI(data);
            const resBox = document.getElementById('bmi-res'); resBox.classList.remove('hidden'); resBox.textContent = `Ваша дневная норма: ${g} ст.`;
        } catch (e) { tg.showAlert(e.message); } finally { btn.classList.remove('btn-loading'); isLocked = false; }
    } else { isLocked = false; tg.showAlert("Введите вес!"); }
}

async function saveHabitsToServer() {
    if(isLocked) return; isLocked = true;
    const btn = document.getElementById('btn-save-habits'); btn.classList.add('btn-loading');
    try {
...
function editHabit(id) {
    ...
    if(h) { document.getElementById('new-h-name').value = h.name; document.getElementById('new-h-cat').value = h.cat; document.getElementById('new-h-xp').value = h.xp; editingId = id; document.getElementById('btn-add-habit').textContent = 'Обновить'; }
}

function addNewHabit() {
    const n = document.getElementById('new-h-name').value.trim(), c = document.getElementById('new-h-cat').value, x = Math.max(1, parseInt(document.getElementById('new-h-xp').value) || 0);
    if(n && x > 0) {
        if(editingId) {
            const i = currentHabits.findIndex(h => h.id === editingId);
            currentHabits[i] = {...currentHabits[i], name: n, cat: c, xp: x}; editingId = null; document.getElementById('btn-add-habit').textContent = 'Добавить в список';
        } else { currentHabits.push({id: 'task-custom-' + Date.now(), cat: c, name: n, xp: x}); }
        document.getElementById('new-h-name').value = ''; document.getElementById('new-h-xp').value = ''; renderHabits();
    } else { tg.showAlert("Заполните корректно название и XP"); }
}
...
function showTab(n) {
    ...
    document.querySelectorAll('nav button').forEach(b => { b.classList.remove('nav-active', 'bg-white/5'); b.classList.add('text-gray-500'); });
    const activeBtn = document.getElementById('nav-' + n); activeBtn.classList.add('nav-active', 'bg-white/5'); activeBtn.classList.remove('text-gray-500');
    if(n === 'tasks' || n === 'history') loadHero(false); 
}

function toggleAccordion(c, i) {
    const el = document.getElementById(c), ic = document.getElementById(i), isOpen = el.style.maxHeight && el.style.maxHeight !== '0px';
    ...
    if(!isOpen) { el.style.maxHeight = el.scrollHeight + "px"; ic.classList.add('rotate-180'); }
}
