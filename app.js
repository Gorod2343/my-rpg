...
tg.expand();
tg.ready(); // Уведомляем Telegram, что приложение полностью загружено!
...
// ---------------------------------------------------------------------------
// Утилиты безопасности (DOM Builder)
// ---------------------------------------------------------------------------
...
async function completeTask(habit, btn) {
    ...
    try {
        ...
        if(habit.id.startsWith('task-custom')) { 
            payload.custom_name = habit.name; 
            payload.custom_xp = habit.xp; 
        }
...
// ---------------------------------------------------------------------------
// Нативная кнопка Telegram (MainButton)
// ---------------------------------------------------------------------------
async function saveHabitsToServer() {
    if(isLocked) return; 
    isLocked = true;
    tg.MainButton.showProgress(); // Показываем крутилку загрузки на кнопке
...
// ---------------------------------------------------------------------------
// Управление UI (Вкладки, Аккордеоны, Модалки)
// ---------------------------------------------------------------------------
function showTab(n) {
...
// Открытие редактора (активирует нативную кнопку Telegram)
function openEditor() { 
    document.getElementById('editor-modal').classList.add('active'); 
    tg.MainButton.setText("СОХРАНИТЬ ПРИВЫЧКИ");
    tg.MainButton.show();
}

// Закрытие редактора (прячет нативную кнопку)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Привязываем событие нативной кнопки Telegram
    tg.MainButton.onClick(saveHabitsToServer);
